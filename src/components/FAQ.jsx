...
const FAQ = () => {
  ...
  return (
    <div className="faq-page min-h-screen p-8 bg-gradient-to-r from-yellow-50 via-orange-50 to-red-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      <h1 className="text-4xl font-extrabold text-orange-600 dark:text-orange-500 mb-8">Frequently Asked Questions</h1>
      <div className="faq-list space-y-4">
        {questions.map((item, index) => (
          <div 
            key={index} 
            className="faq-item border-b border-gray-300 dark:border-gray-700 pb-4 dark:hover:border-orange-500 transition-colors duration-300"
          >
            <h2 
              className="text-2xl font-bold text-gray-900 dark:text-gray-100 cursor-pointer dark:hover:text-orange-500 transition-colors duration-300" 
              onClick={() => toggleAnswer(index)}
            >
              {item.question}
            </h2>
            {activeIndex === index && (
              <p className="text-gray-700 dark:text-gray-300 mt-2 dark:pl-4 dark:border-l-2 dark:border-orange-500">
                {item.answer}
              </p>
            )}
          </div>
